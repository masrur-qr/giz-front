        ...
        if (map && !controlRef.current) {
            ...
            const customControl = L.Control.extend({
                onAdd: function () {
                    ...
                    div.innerHTML = `<button style="width: 30px; padding: 2px;"><img style="width: 100%" src="${layer1.src}"</button>`;
                    div.onclick = onClick;
                    return div;
                },
            });
            const controlInstance: any = new customControl({ position: 'bottomright' });
    ...
    useEffect(() => {
        if (map && !controlRef.current) {
            ...
            const customControl = L.Control.extend({
                onAdd: function () {
                    const div = L.DomUtil.create('div', 'leaflet-bar leaflet-control leaflet-control-custom');
                    div.innerHTML = `<button style="width: 30px; padding: 2px;"><img style="width: 100%" src="${layer2.src}"</button>`;
                    div.onclick = onClick;
                    return div;
                },
            });
...
export default function IntMap(props: any) {
    ...
    const handleSelectFilter = (value: number) => {
        ...
        let districtName = "";
        districtsMap.features.forEach((el: { properties: { district: number; district_name: string; }; }) => { if (el.properties.district == value) districtName = el.properties.district_name })
                ...
                >
                    <Popup className="w-[300px]  ">
                        <div className="flex flex-col ">
                            ...
                            <div className="">
                                <h5 className="text-lg">{project.name_en}</h5>
                                <div className="flex gap-1">
                                    ...
                                    <p>{districtsMap.features.filter((el: any) => { if (el.properties.district == project.district_id) return el })[0].properties.name}</p>
                                </div>
                                <button
